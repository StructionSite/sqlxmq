...
CREATE OR REPLACE FUNCTION mq_poll(channel_names TEXT[], batch_size INT DEFAULT 1, max_retry_text TEXT DEFAULT NULL)
RETURNS TABLE(
    id UUID,
...
BEGIN
    ...
    SET
        ...
        retry_backoff = CASE WHEN max_retry_text IS NULL THEN mq_msgs.retry_backoff * 2 ELSE LEAST(mq_msgs.retry_backoff * 2,  max_retry_text::interval) END
    FROM (
        SELECT
            msgs.id
